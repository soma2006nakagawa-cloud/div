let map,shelters=[],currentLat,currentLng,currentMarker,nearestMarker,routeLine,allMarkers=[];
const nearestInfo=document.getElementById("nearestInfo"),top5=document.getElementById("top5");
function distance(lat1,lng1,lat2,lng2){const R=6371,dLat=(lat2-lat1)*Math.PI/180,dLng=(lng2-lng1)*Math.PI/180,a=Math.sin(dLat/2)**2+Math.cos(lat1*Math.PI/180)*Math.cos(lat2*Math.PI/180)*Math.sin(dLng/2)**2;return R*2*Math.atan2(Math.sqrt(a),Math.sqrt(1-a));}
fetch("data.json").then(res=>{if(!res.ok)throw new Error("data.jsonが見つかりません");return res.json();}).then(data=>{shelters=data.shelters||[];if(shelters.length===0)throw new Error("避難所データがありません");getLocation();}).catch(err=>{nearestInfo.innerHTML=`<p class="error">${err.message}</p>`;});
function getLocation(){if(!navigator.geolocation){nearestInfo.innerHTML='<p class="error">このブラウザは位置情報に対応していません。</p>';return;}navigator.geolocation.getCurrentPosition(pos=>{currentLat=pos.coords.latitude;currentLng=pos.coords.longitude;findNearest();},()=>{nearestInfo.innerHTML='<p class="error">位置情報が取得できません。ブラウザの位置情報を許可してください。</p>';},{enableHighAccuracy:true,maximumAge:0,timeout:10000});}
